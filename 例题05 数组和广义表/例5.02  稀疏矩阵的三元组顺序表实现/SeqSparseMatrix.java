...
public class SeqSparseMatrix
{
    ...
    public boolean add(Element item)             //按行主序插入一个元素的三元组
    {                                            //
        ...
        while (i<this.list.length() && item.compareTo(temp)>0) //item“大”时向后走
        {
            i++;
    ...
    public SeqSparseMatrix transpose()                     //返回转置矩阵
    {
        ...
        for (int i=0; i<this.list.length(); i++)
            trans.add(this.list.get(i).toSymmetry());      //插入矩阵对称位置元素的三元组
        return trans;
    }
