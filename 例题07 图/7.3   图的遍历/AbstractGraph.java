...
import dataStructure.linearList.SeqQueue;                  //顺序循环队列类

public abstract class AbstractGraph<E> implements GGraph<E>//抽象图类 
{
    public abstract int vertexCount();                     //返回顶点数，方法由子类实现
    public abstract E get(int i);                          //返回顶点vi的数据域
    public abstract int getFirstNeighbor(int i);           //返回顶点vi的第一个邻接顶点的序号
    public abstract int getNextNeighbor(int i, int j);     //返回vi在vj后的下一个邻接顶点的序号  
//    public abstract AbstractGraph prim();


    public void DFSTraverse(int v)                         //从顶点v出发对非连通图的一次深度优先搜索遍历
    {
    ...
    private void depthfs(int v, boolean[] visited)         //从顶点v开始发的一次深度优先搜索遍历
    {                                                      //遍历一个连通分量
    ...
    public void BFSTraverse(int v)                         //从顶点v出发对非连通图进行一次广度优先搜索遍历
    {
    ...
    private void breadthfs(int v, boolean[] visited)       //从顶点v出发的广度优先搜索遍历
    {                                                      //遍历一个连通分量
        System.out.print(this.get(v)+" ");
        visited[v] = true;
        SeqQueue<Integer> que = new SeqQueue<Integer>(vertexCount());   //创建顺序队列
...
public class Graph3 extends Graph1       //图的遍历 
{
    protected int tree[][];              //图的生成树的邻接矩阵 
    Graph3(int m1[][])                   //以邻接矩阵表示图
    {
        super(m1);
        tree = new int [n][n];
    }
    ...
    public void prim()
    {
        ...
        int n=0;
        while (n<this.mat.length-1)             //找出权值最小的边
        {
                min=10000;
                min_i=0;
                min_j=0;
    ...
    {
        int n=5,k;
        int mat3[][] = {{0,2,4,5,0},         //无向带权图的邻接矩阵 
                        {2,0,8,0,9},
                        {4,8,0,6,3},
                        {5,0,6,0,7},
                        {0,9,3,7,0}};
        Graph3 g3 = new Graph3(mat3);
        System.out.print("生成树的");
        g3.prim();
    }
}
...
/*
        ...
        System.out.println("广度优先遍历Breath first search:");
        for (k=0;k<n;k++)
        {
            g3.breadthfs(k);
            System.out.println();
