import dataStructure.linearList.SeqStack;                  //顺序栈类
import dataStructure.linearList.LinkedStack;               //链式栈类

public class Expression 
{
    public static String toPostfix(String expstr)          //返回expstr的后缀表达式
    {
        SeqStack<String> stack = new SeqStack<String>(expstr.length());   //创建运算符栈
        String postfix="";
        int i=0;
        ...
        {    
            ...
            switch (ch)
            {
                ...
                case '(': stack.push(ch+"");               //遇到左括号，入栈
                          i++; 
                          break;
                case ')': String out = stack.pop();        //遇到右括号，出栈
                          while (out!=null && !out.equals("("))  //若out==null，表示栈空
                          {
                              postfix += out;
                              out = stack.pop();
                          }
...
//                          break;
    ...
    {
        LinkedStack<Integer> stack = new LinkedStack<Integer>();     //创建操作数栈
        int i=0, result=0;
